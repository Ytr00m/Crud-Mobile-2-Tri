...
class _FormProdutosPageState extends State<FormProdutosPage> {
  ...
  Widget build(BuildContext context) {
    ...
    salvar() {
      setState(() {
        if(index > -1) {
          ...
          produtos[index].estoque = int.parse(_estoque.text) ?? 0;
        }

        else {
          produtos.add(Produto(_nome.text, _descricao.text, double.parse(_preco.text), int.parse(_estoque.text)));
        }
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Form Produtos"), backgroundColor: Theme.of(context).primaryColor,),
